...
public class Task1 {
     ...
     * @throws NullPointerException if points is null or if any
     *         entry in points[] is null
     */
    public Task1(Point2D[] points) {
        int N = points.length;
        if (N <= 1) return;
   ...
   /**
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Enter pair of point coordinates by a space or newline");
        for (int i = 0; i < N; i++) {
            double x = input.nextDouble();
            double y = input.nextDouble();
            points[i] = new Point2D(x, y);
        }
        Task1 closest = new Task1(points);
